...
def initialize_model(init_model,model_type,which_model):
    ...
    # Stardist
        ...
        if which_model == "SD_dsb_versatile":
            model = StarDist2D(None, name='dsb_versatile', basedir = init_model["basedir_StarDist"])
            mds["SD_dsb_versatile"] = model
        if which_model == "SD_patho":
            model = StarDist2D(None, name='patho', basedir=init_model["basedir_StarDist"])
            mds["SD_patho"] = model
    # Cellpose
